...
class Router
{
  ...
  public function addRoute($httpMethod, $routePath, $routeName, $handler)
  {
    ...
    if (!empty( $homePath['path'] )) {
      $routePath = $homePath['path'] . $routePath;
    }
   ...
   * @param  callback  $handler    The route's handler
   */
  public function get($routePath, $handler , $routeName = NULL)
  {

    if (is_null($routeName)) {
      $routeName = sha1($routePath);
    }
   ...
   * @param  callback  $handler    The route's handler
   */
  public function post($routePath, $handler , $routeName = NULL)
  {
    if (is_null($routeName)) {
      $routeName = sha1($routePath);
    }
