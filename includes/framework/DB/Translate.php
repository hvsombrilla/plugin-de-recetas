...
class Translate {
	...
	public function select( $statements ) {
		...
		$customwhere = isset( $statements['customwhere'] ) ? $statements['customwhere'] : '';
		// Having
	...
	protected function buildJoin( $statements ) {
		...
		foreach ( $statements['joins'] as $joinArr ) {
			...
			$joinBuilder = $joinArr['comparation'];
			$sqlArr = array( $sql, strtoupper( $joinArr['type'] ), 'JOIN', $table, 'ON', $joinBuilder );
