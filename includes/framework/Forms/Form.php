<?php
namespace SombrillaWP\Forms;

use SombrillaWP\Forms\Fields\Submit;
use SombrillaWP\Forms\Traits\MacroTrait;
use SombrillaWP\Html\Generator;
use SombrillaWP\Html\Tag;
use SombrillaWP\Forms\Fields\Field;
use SombrillaWP\Models\WPOption;
use SombrillaWP\Models\WPPost;
use SombrillaWP\Models\WPTerm;
use SombrillaWP\Models\Model;
use SombrillaWP\Forms\Traits\FormConnectorTrait;
use SombrillaWP\Register\Registry;
use SombrillaWP\Utility\Str;
...
class Form
{
    ...
    /** @var \SombrillaWP\Forms\Fields\Field $currentField */
    ...
    public function __construct( $resource = 'auto', $action = 'update', $itemId = null )
    {
        ...
        $model = "\\SombrillaWP\\Models\\{$Resource}";
    ...
    protected function autoConfig()
    {
        if ($this->resource === 'auto') {
            ...
            if ( isset( $post->ID ) ) {
                ...
                if( empty($resource) || ! class_exists($model) ) {
                    $resource = 'post';
                ...
                $resource = $resource[0];
                $model = "\\SombrillaWP\\Models\\{$Resource}";
                $controller = "\\SombrillaWP\\Controllers\\{$Resource}Controller";
     ...
     * @param $dots
     *
     * @return \SombrillaWP\Forms\Form $this
     */
    ...
    public function open( $attr = [] )
    {
        ...
        $r .= wp_nonce_field( 'form_' .  'sombrilla' , '_tr_nonce_form', false, false );
     ...
     * @return Fields\Textarea
     */
    public function button( $name, array $attr = [], array $settings = [], $label = false )
    {
        return new Fields\Button( $name, $attr, $settings, $label, $this );
    }
