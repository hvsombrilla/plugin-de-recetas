...
function save_swp_post_meta( $post_id, $post, $update ) {
...
add_action( 'save_post', 'save_swp_post_meta', 10, 3 );

function swp_banners($id = NULL){
    return new SombrillaWP\Models\Banners($id);
}

function swp_postmeta($post = NULL, $key = NULL , $prefix = 'swp'){
    if (is_null($post)) {
       $post = get_the_ID();
    }

    if (!empty(get_post_meta($post, $prefix, true))) {
        if (is_null($key)) {
                return get_post_meta($post, $prefix, true);
        }else{
                return get_post_meta($post, $prefix, true)[$key];
        }
...
function swp_option($key , $prefix = 'swp'){
    if (!empty(get_option($prefix, true)[$key])) {
       return get_option($prefix, true)[$key];
    }
...
// if (isset($_POST['swp'])) {
//     foreach ($_POST['swp'] as $field => $value) {
//         update_post_meta($_POST['post_ID'], $field, $value);
//     }
// }


function sombrilla_db () {
    return new SombrillaWP\DB\Query();
}
...
if( ! function_exists('sombrilla_taxonomy') ) {
/**
 * Register taxonomy
 *
 * @param $singular
 ...
 */
function sombrilla_taxonomy(
    $singular,
    $settings = [],
    $plural = null
    
) {
    $obj = new SombrillaWP\Register\Taxonomy($singular, $plural, $settings);
    $obj->addToRegistry();
...
if( ! function_exists('sombrilla_post_type') ) {
/**
 * Register post type
 *
 ...
 */
function sombrilla_post_type(
    $singular,
    $settings = [],
    $plural = null
) {
    $obj = new SombrillaWP\Register\PostType($singular, $plural, $settings);
    $obj->addToRegistry();
...
if( ! function_exists('sombrilla_meta_box') ) {
/**
 * Register meta box
 *
 * @param null $name
 ...
 */
function sombrilla_metabox(
    $name = null,
    $screen = null,
    $settings = []
) {
    $obj = new SombrillaWP\Register\MetaBox($name, $screen, $settings);
    $obj->addToRegistry();
 ...
 * @param array $settings
 *
 * @return \TypeRocket\Register\Page Commet
 */
...
if( ! function_exists('sombrilla_tabs') ) {
/**
 * Create tabs
 *
 * @return \TypeRocket\Elements\Tabs
 */
    function sombrilla_tabs()
    {
        return new SombrillaWP\Forms\Tabs();
    }
}
 ...
 */
    function sombrilla_form($resource = 'auto', $action = 'update', $item_id = null)
    {
        return new \SombrillaWP\Forms\Form($resource, $action, $item_id);
    }
}
...
if( ! function_exists('sombrilla_route') ) {
/**
 * Route
 *
 * @return \TypeRocket\Http\Route
 */
function sombrilla_route()
{
...
// Create the router instance
    return JetRouter\Router::create($config);
}
}
